...
struct HashTable::Node {
  ...
  Node(const Node &b) : data(new Value) {
    const Node *tmp = &b;
    Node *pointer = this;
...
HashTable::HashTable()
    : capacity(initial_capacity), table(new Node *[initial_capacity]) {
...
HashTable::HashTable(size_t size)
    : capacity(size), table(new Node *[capacity]) {
  std::fill(&table[0], &table[capacity], nullptr);
}

HashTable::HashTable(const HashTable &b)
    : curr_size(b.curr_size), capacity(b.capacity),
...
HashTable::HashTable(HashTable &&b) noexcept
    : curr_size(b.curr_size), capacity(b.capacity), table(b.table) {
  b.table = nullptr;
}

void HashTable::swap(HashTable &b) noexcept { std::swap(b, *this); }
...
bool operator==(const HashTable &a, const HashTable &b) {
  ...
  for (int i = 0; i < b.capacity; i++) {
    ...
    if (b.table[i]->key.empty() && !b.table[i]->flag)
      continue;
    while (current != nullptr) {
      try {
        if (b.at(current->key) != a.at(current->key))
          return false;
...
size_t HashTable::hashFunction(const Key &key) const noexcept {
  if (testing_mode) {
    return hashFunctionForTest();
  }
  size_t bucketIndex = 0;
  size_t sum = 0;
  size_t factor = default_factor;
  for (char i : key) {
    sum = ((sum % capacity) + ((static_cast<int>(i)) * factor) % capacity) %
          capacity;
    factor = ((factor % __INT16_MAX__) * (default_factor % __INT16_MAX__)) %
             __INT16_MAX__;
  }
  bucketIndex = sum;
  return bucketIndex;
